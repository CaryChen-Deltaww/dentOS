...
plat_info_t gPlat_info[] = {
    [PID_B30GE6X1] = {
        .name = "arm64-delta-b30ge6x1",

        .onie_eeprom_path = "/sys/bus/i2c/devices/0-0051/eeprom",
...
plat_id_t get_platform_id(void)
{
    ...
    plat_info_t *b30ge6x1_info = &gPlat_info[PID_B30GE6X1];

    if (plat_os_file_is_existed(b30ge6x1_info->cpld_path)) {
        sprintf(pid_fullpath, "%s/platform_id", b30ge6x1_info->cpld_path);
    } else {
        return PID_UNKNOWN;
    }
    ...
    if (pid >= PID_UNKNOWN || pid < PID_B30GE6X1) {
        return PID_UNKNOWN;
    }
